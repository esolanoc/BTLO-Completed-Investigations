⚡BTLO Completed Investigations (Retired)

🚀 A collection of forensic investigations and technical reports conducted in BTLO laboratory environments.

<div style="border:2px solid #4CAF50; border-radius:10px; padding:15px; background:#f9f9f9; font-family:Arial, sans-serif; text-align:center;">

  <h2 style="color:#4CAF50; margin-bottom:10px;">📊 Progress Overview</h2>

  <p style="font-size:18px; margin:5px 0;">
    <strong>Investigations Completed:</strong> 
    <span style="color:#2196F3;">29 / 273</span>
  </p>

  <p style="font-size:18px; margin:5px 0;">
    <strong>Challenges Completed:</strong> 
    <span style="color:#FF9800;">22 / 41</span>
  </p>

  <div style="margin-top:15px;">
    <progress value="29" max="273" style="width:80%; height:20px;"></progress>
    <br>
    <progress value="22" max="41" style="width:80%; height:20px;"></progress>
  </div>

</div>




<img width="1818" height="707" alt="image" src="https://github.com/user-attachments/assets/7c43b7f2-050b-4d0f-ae68-7c9068b8068b" />


